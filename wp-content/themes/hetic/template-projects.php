...
<?php get_header(); ?>
<main class="main exhibitions" id="projects">
    <div class="full-image">
        <img src="<?php echo IMAGES_URL ?>/JR-exhib.png" alt="JR Projects">
    </div>
    <div class="container">
        <header class="header-scroll scroll-disable">
            <ul class="header-scroll-nav">
                <li v-for="project in projects">
                    <button class="list-years" @click="setActiveProject(project.name, project.slug)"
                            :class="{active:activeProjectSlug == project.slug}">{{ project.name }}
                    </button>
                </li>
            </ul>
            </ul>
        ...
        <div class="row">
            <div class="item-container menu">
                <h4 class="item-title menu-title">{{ activeProject }}</h4>
                <ul class="list-exhibtions">
                    <li class="exhibition-item" v-for="projectsDetail in projectsDetails">
                        <button class="exhibition-item-link" @click="getDetails(projectsDetail.ID)">{{
                            projectsDetail.post_title }}
                        </button>
                    </li>
                </ul>
            </div>
        ...
        <div class="row-slider slider" id="slider-p"
             v-if="!loaderArticle && article.photos && article.photos.length > 0">
            <div class="slider-container">
                <div class="slider-item" v-for="photo in article.photos">
                    <div class="slider-img" :style="{backgroundImage: `url(${photo.image.url})`}"></div>
                </div>
            </div>
        ...
        <div class="section-videos-nav"
            ...
            <div class="section-videos-nav-wrapper">
                <div class="section-videos-nav-wrapper-list">
                    <button v-for="video in article.videos" :data-target="video.video.url"
                            :style="{backgroundImage: video.video.url}"
